...
edebilirken, sadece bir sınıfı extend edebiliyor olmasının sebebi çelişkili durumların ortaya çıkabilmesinden kaynaklıdır.
...
Test sınıfından bir nesne ürettiğimizde ve run() fonksiyonunu çağırdığımızda bir derleme hatası alırız. Çünkü kod hangi run() metodunun çağrılacağını bilemez.
Parent1'in run() metodu mu yoksa Parent2'nin run() metodu mu çağrılmalı? Burada bir karmaşıklık oluşur.
Bu nedenle bu karmaşıklığa sebep olunmaması için Java birden fazla snıf extends etmez.
